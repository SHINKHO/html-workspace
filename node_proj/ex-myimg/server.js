...
let saltRounds = 10; // 단순 해쉬알고리즘에 반복적으로 더 복잡하게 만들기위해 반복횟수? 넣은듯
...
app.post('/signup', async(req, res)=>{
    ...
    try {
        ...
        res.render('login', {id : user_id, msg : '회원가입 되셨습니다. 로그인 진행해 주세요!!'});
    } catch (error) {
...
app.post('/login', async(req, res)=>{
    ...
    try{
        ...
        if(result.rows.length > 0) {
            ...
            res.render('login', {msg:'잘못된 회원 아이디입니다.', id:''});
        }
    } catch(e) {
        console.log(e);
    }
...
app.post('/upload', upload.single('myfile'), async(req, res)=>{  // 여기서 저장은 됨??
    const file = req.file;
    const meta = {
    ...
    try {
        ...
        console.log('insert rowcount : '+ result.rowsAffected);
    } catch (err) {
        console.log(err);
    }
    res.render('main', {msg: '저장되었습니다.', id:req.body.user_id});
});
